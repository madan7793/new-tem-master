...
const Cart = () => {
    ...
    if (activeCartProducts.length !== 0) {
        const eachTotal = activeCartProducts.map(each => {
            if (each.productdiscountPrice) {
                return each.productCount * each.productdiscountPrice
            } else {
    ...
    const onClickConfirmOrder = async () => {
        if (transactionId.length === 23) {
            ...
            try {
                const { data } = await placeOrder(activeAddress.fullAddress, userData, cartProducts, total, transactionId)
                if (data.response) {
                    ...
                    <h5 className="order-summary">ORDER SUMMARY</h5>
                        ...
                        <div className="bill">
                            ...
                            <div className="transaction-container">
                                ...
                                <button className="confirm-btn" onClick={onClickConfirmOrder} type="button">CONFIRM ORDER</button>
                        </div>
                    </div>
            </div>
        </div>
                    ...
                    <div className="form-address-payment">
                        ...
                        <div className="payment-details">
                            ...
                            <div className="bill">
                                ...
                                <div className="bottom-btn-container">
                                    <button type="button" disabled={activeAddressKey === "" || activeAddressKey === undefined} className="continue-btn" onClick={() => setContent(contentConst.payment)}>CONFIRM ADDRESS</button>
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        <>
            {cartProducts.length === 0 ? (
                <div className="empty-cart-container">
                    ...
                    <Link to="/"><button type="button" className="wishlist-btn">ADD ITEMS FROM WISHLIST</button></Link>
                </div>
            ) : (
                <>
                    <hr className="horizontal-line"></hr>
                    <div className="cart-items-bg-container">
                        <div className="cart-items-container">
                            ...
                            <div className="wishlist-button">
                                <div className="wishlistbtn">Add more from Wishlist 
                                    <BsHeart/>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="address-payment-details">
                            ...
                            <div className="payment-details">
                                ...
                                <div className="bottom-btn-container">
                                    <button type="submit" disabled={selectedItemsLength === 0} className="confirm-btn" onClick={onClickContiue}>PLACE ORDER</button>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    return (
        <div className="cart-container">
            ...
            <div className="progressBar">
                <h1 className={`progress-item ${content === contentConst.cart && 'active'}`} onClick={() => setContent(contentConst.cart)}>Bag</h1><hr className="dash-line" /><h1 className={`progress-item ${content === contentConst.address && 'active'}`} onClick={() => {if(selectedItemsLength !== 0 && userLogged) { setContent(contentConst.address)}}}>Address</h1><hr className="dash-line" /><h1 className={`progress-item ${content === contentConst.payment && 'active'}`} onClick={() =>{if(selectedItemsLength !== 0 && userLogged && activeAddressKey !== undefined){ setContent(contentConst.payment)}}} >Payment</h1>
            </div>
            {renderContent()}
            <hr className="line" />
