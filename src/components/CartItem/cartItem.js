...
const CartItem = ({cartItemData}) => {
    const {shopId, productId, productImages, productName, productDescription, productPrice, hasVariants, size, color, checked, productdiscountPrice, productCount} = cartItemData
    let miniDescription = "";
    ...
    const date = new Date()
    const expectedDeliveryDates = add(new Date(date), {days: 7}).toDateString().split(' ')
                ...
                <div className='cart-item-details-container'>
                    ...
                    <div className="size-color-shop-contollers-container">
                        <div className="size-color-shop-container">
                            {hasVariants ? <div className="size-color-container"><div className="size-container"><p className="size-h">Size :</p><p className="size">{size}</p></div><div className="color-container"><p className="color-h">Colo :</p><div className="Color-ind" style={{backgroundColor: `${color}`}}></div></div></div> : <p className="seller-name">Sold by: {shopId}</p>}
                        </div>
                    </div>
                    ...
                    {productdiscountPrice && <p className='product-price'>Rs : {productdiscountPrice}</p>} 
                    {(!productdiscountPrice && productPrice) && <p className='product-price'>Rs : {productPrice}</p>}
                    <p className="delivery-date">Delivery till: {`${expectedDeliveryDates[2]}th ${expectedDeliveryDates[1]} ${expectedDeliveryDates[3]}`}</p>
                </div>
            </div>
